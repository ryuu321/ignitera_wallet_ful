    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-20">
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Welcome back, {user.anonymousName}</p>
        </div>
        <div className="flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-full">
          <ShieldCheck className={user.role === 'MANAGER' ? "text-red-400" : "text-green-400"} size={20} />
          <span className="text-sm font-medium">{user.role} | Rank {user.rank}</span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {/* Balances */}
        <BalanceCard title="Market (C_flow)" amount={user.balanceFlow} color="blue" />
        <BalanceCard title="Assets (C_stock)" amount={user.balanceStock} color="purple" />
        <BalanceCard title="Investment (IGN)" amount={user.balanceIgn} color="amber" />
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <div className="flex justify-between mt-2 text-xs text-gray-500">
              <span>JAN</span><span>FEB</span><span>MAR</span><span>APR</span><span>MAY</span><span className="text-green-400">JUN</span>
            </div>
          </section>
          ...
          <section>
            ...
            {tasks.length > 0 ? (
                ...
                {tasks.map(t => (
                  <div key={t.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex justify-between items-center transition-transform hover:scale-[1.01]">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">Due: 2 days left</p>
                    </div>
              ...
              <div className="bg-white/5 border border-white/10 border-dashed rounded-xl p-8 text-center text-gray-500">
                No active missions. Check the marketplace.
              </div>
            )}
          </section>
        </div>
        ...
        <div className="space-y-8">
              ...
              {recentTransactions.map(tx => {
                ...
                return (
                  <div key={tx.id} className="flex gap-4">
                    ...
                    <div>
                      <div className="font-medium">{isIncoming ? `From ${tx.fromUser.anonymousName}` : `To ${tx.toUser.anonymousName}`}</div>
          ...
          <section className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-6">
            ...
            <p className="text-sm text-amber-200/60 leading-relaxed">
              Your recent bid for "Frontend Maintenance" is still pending approval by Vanguard-Lead.
            </p>
          </section>
        </div>
      </div>
